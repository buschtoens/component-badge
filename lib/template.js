...
function parse(template) {
	var strings = []
		, variables = []
		, match = template.match(/([^]*?)\$\{(\w+)(?:\:(\w+))?\}([^]*)/im); // Lorem ${myVar:default} ipsum...
...
Template.prototype.render = function render(variables) {
	var rendered = "";
	console.log(this.strings, this.variables)
	this.strings.forEach(function(string, index) {
		rendered += string;
		rendered += variables[this.variables[index].name] || this.variables[index].def || "";
	}, this);
